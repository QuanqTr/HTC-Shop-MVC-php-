...
{
    $action = $_GET['action'] ?? null; // Kiểm tra nếu `action` không tồn tại
    $classModel = Application::$app->session->get('confirmTemp');

    if (!$classModel) {
        // Nếu `$classModel` không tồn tại, xử lý lỗi
        Application::$app->session->setFlash('errors', ['Không tìm thấy dữ liệu để xử lý.']);
    ...
    try {
        $now = new DateTime();
        $classModel->updated_at = $now->format('Y-m-d H:i:s'); // Gán thời gian hiện tại

        if ($action === 'create') {
            if ($classModel->validate() && $classModel->save()) {
                // Xóa dữ liệu tạm thời và hiển thị thông báo thành công
                Application::$app->session->remove('confirmTemp');
        ...
        Application::$app->session->setFlash('errors', [$e->getMessage()]);
        Application::$app->response->redirect('/admin/products');
    }
}
